...
test "destructive iterator compacts once and reuses compact bytes" {
    var source = "<r a='1' b=\"two\"/>".*;
    ...
    try std.testing.expectEqualStrings("1", a.value.slice(&source));
    ...
    var second = Iterator(false, false).initElement(&source, name_end);
    _ = second.next() orelse return error.TestUnexpectedResult;
    _ = second.next() orelse return error.TestUnexpectedResult;
...
test "destructive materialization declines malformed literal NUL" {
    ...
    const before = source;
    try std.testing.expect(!materialize(false, &source, 2));
